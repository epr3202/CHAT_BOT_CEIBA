"""R5 launcher reusing frozen isolation, with an incremental R4-based allowlist."""
...
BASE = "5d093f73f038f41ce683a5a1f2b4194a0426af1f"
R4_BASE = "5d093f73f038f41ce683a5a1f2b4194a0426af1f"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r5-h03-media-"):
    ...
    network, database, runner = [f"ceiba-r5-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r5=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r5") != nonce:
    ...
    try:
        ...
        run("git", "merge-base", "--is-ancestor", R4_BASE, actual)
        changed = run("git", "diff", "--name-only", R4_BASE, actual).stdout.decode().splitlines()
        ...
        if baseline["r4_base_sha"] != R4_BASE:
            raise RuntimeError("Wrong R4 baseline")
        protected = {}
        ...
        manifest["r4_base_sha"] = R4_BASE
        root = Path(tempfile.mkdtemp(prefix="ceiba-r5-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r5.driver"]\n'
        )
        image = "ceiba-quality-r5:" + nonce
