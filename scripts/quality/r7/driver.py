"""R7 candidate tests with retained R1/R2/R3 nodes and independent Alembic regressions."""
...
def main() -> None:
    ...
    try:
        ...
        required = set(json.loads(Path("scripts/quality/r7/r6_nodes.json").read_text()))
        ...
        required_r7 = set(json.loads(Path("scripts/quality/r7/new_nodes.json").read_text()))
        discovered_r7 = {n for n in result["nodes"] if n.startswith("tests/remediation/r7/")}
        summary["new_r7_nodes"] = sorted(discovered_r7)
        summary["missing_red_nodes"] = sorted(required_r7 - discovered_r7)
        expected_nodes |= discovered_r7 | required_r7
