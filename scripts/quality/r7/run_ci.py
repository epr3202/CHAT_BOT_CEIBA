"""R7 launcher reusing frozen isolation, with an incremental R4-based allowlist."""
...
BASE = "cfdc09b1d2805da5743e3f6466cb88fcb3142905"
PARENT_BASE = "cfdc09b1d2805da5743e3f6466cb88fcb3142905"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r7-h29-entities-"):
    ...
    network, database, runner = [f"ceiba-r7-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r7=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r7") != nonce:
    ...
    try:
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r7-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r7.driver"]\n'
        )
        image = "ceiba-quality-r7:" + nonce
        ...
        for offset in range(0, len(encoded), 12000):
            print("R7_ZIP:" + encoded[offset:offset + 12000])
        print("R7_ZIP_SHA256:" + hashlib.sha256(archive.read_bytes()).hexdigest())
        print(
            json.dumps(
                {
                    k: manifest.get(k)
