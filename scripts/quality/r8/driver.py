"""R8 candidate tests with retained R1/R2/R3 nodes and independent Alembic regressions."""
...
def main() -> None:
    ...
    try:
        ...
        required = set(json.loads(Path("scripts/quality/r8/r7_nodes.json").read_text()))
        ...
        required_r8 = set(json.loads(Path("scripts/quality/r8/new_nodes.json").read_text()))
        discovered_r8 = {n for n in result["nodes"] if n.startswith("tests/remediation/r8/")}
        summary["new_r8_nodes"] = sorted(discovered_r8)
        summary["missing_red_nodes"] = sorted(required_r8 - discovered_r8)
        expected_nodes |= discovered_r8 | required_r8
