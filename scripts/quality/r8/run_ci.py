"""R8 launcher reusing frozen isolation, with an incremental R4-based allowlist."""
...
BASE = "9af02027672049c1d73f998e9be1fe8c070e791f"
PARENT_BASE = "9af02027672049c1d73f998e9be1fe8c070e791f"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r8-h11-ownership-"):
    ...
    network, database, runner = [f"ceiba-r8-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r8=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r8") != nonce:
    ...
    try:
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r8-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r8.driver"]\n'
        )
        image = "ceiba-quality-r8:" + nonce
        ...
        for offset in range(0, len(encoded), 12000):
            print("R8_ZIP:" + encoded[offset:offset + 12000])
        print("R8_ZIP_SHA256:" + hashlib.sha256(archive.read_bytes()).hexdigest())
        print(
            json.dumps(
                {
                    k: manifest.get(k)
