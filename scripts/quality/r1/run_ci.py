...
BASE = "49fb61eb35a59b1142d4b2b81aff84fffc8db277"
R0_BASE = "49fb61eb35a59b1142d4b2b81aff84fffc8db277"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r1-h02-outbox-"):
    ...
    network, database, runner = [f"ceiba-r1-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r1=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r1") != nonce:
    ...
    try:
        ...
        run("git", "merge-base", "--is-ancestor", R0_BASE, actual)
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r1-"))
        source = root / "source"
        source.mkdir()
        ...
        for rel in changed:
            if rel.startswith(("app/", "tests/", "scripts/quality/r1/", "alembic/")):
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r1.driver"]\n'
        )
        image = "ceiba-quality-r1:" + nonce
