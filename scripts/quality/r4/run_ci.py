"""R4 launcher reusing frozen isolation, with an incremental R3-based allowlist."""
...
BASE = "b696d333034e3631612fcb203d66d6953a8cbe08"
R3_BASE = "b696d333034e3631612fcb203d66d6953a8cbe08"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r4-h05-explicit-human-"):
    ...
    network, database, runner = [f"ceiba-r4-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r4=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r4") != nonce:
    ...
    try:
        ...
        run("git", "merge-base", "--is-ancestor", R3_BASE, actual)
        changed = run("git", "diff", "--name-only", R3_BASE, actual).stdout.decode().splitlines()
        ...
        if baseline["r3_base_sha"] != R3_BASE:
            raise RuntimeError("Wrong R3 baseline")
        protected = {}
        ...
        manifest["r3_base_sha"] = R3_BASE
        root = Path(tempfile.mkdtemp(prefix="ceiba-r4-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r4.driver"]\n'
        )
        image = "ceiba-quality-r4:" + nonce
