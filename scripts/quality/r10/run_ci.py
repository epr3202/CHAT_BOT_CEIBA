"""R10 launcher reusing frozen isolation, with an incremental R4-based allowlist."""
...
BASE = "a9ce7f34342f19022ae608ab48bad4b0654b52d2"
PARENT_BASE = "a9ce7f34342f19022ae608ab48bad4b0654b52d2"
...
def main() -> None:
    ...
    network, database, runner = [f"ceiba-r10-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r10=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r10") != nonce:
    ...
    try:
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r10-"))
        source = root / "source"
        source.mkdir()
        ...
        for rel in changed:
            if rel.startswith(("app/", "tests/", "scripts/", "alembic/", "frontend/")):
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r10.driver"]\n'
        )
        image = "ceiba-quality-r10:" + nonce
        ...
        for offset in range(0, len(encoded), 12000):
            print("R10_ZIP:" + encoded[offset:offset + 12000])
        print("R10_ZIP_SHA256:" + hashlib.sha256(archive.read_bytes()).hexdigest())
        print(
            json.dumps(
                {
                    k: manifest.get(k)
