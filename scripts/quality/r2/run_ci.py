...
BASE = "66a398124c007bdc54895fe891601e470910ce8e"
R1_BASE = "66a398124c007bdc54895fe891601e470910ce8e"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r2-h01-inbox-"):
    ...
    network, database, runner = [f"ceiba-r2-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r2=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r2") != nonce:
    ...
    try:
        ...
        run("git", "merge-base", "--is-ancestor", R1_BASE, actual)
        changed = run("git", "diff", "--name-only", R1_BASE, actual).stdout.decode().splitlines()
        ...
        manifest["r1_base_sha"] = R1_BASE
        root = Path(tempfile.mkdtemp(prefix="ceiba-r2-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r2.driver"]\n'
        )
