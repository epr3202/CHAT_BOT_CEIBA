"""R3 launcher reusing frozen isolation, with an incremental R2-based allowlist."""
...
BASE = "163272a60260641c3e7780105ee2841d9adfba3f"
R2_BASE = "163272a60260641c3e7780105ee2841d9adfba3f"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r3-h04-ai-"):
    ...
    network, database, runner = [f"ceiba-r3-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r3=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r3") != nonce:
    ...
    try:
        ...
        run("git", "merge-base", "--is-ancestor", R2_BASE, actual)
        changed = run("git", "diff", "--name-only", R2_BASE, actual).stdout.decode().splitlines()
        ...
        if baseline["r2_base_sha"] != R2_BASE:
            raise RuntimeError("Wrong R2 baseline")
        protected = {}
        ...
        manifest["r2_base_sha"] = R2_BASE
        root = Path(tempfile.mkdtemp(prefix="ceiba-r3-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r3.driver"]\n'
        )
        image = "ceiba-quality-r2:" + nonce
