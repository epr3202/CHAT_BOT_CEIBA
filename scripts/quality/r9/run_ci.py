"""R9 launcher reusing frozen isolation, with an incremental R4-based allowlist."""
...
BASE = "068815555a266b784bafc8dd521ca618eabbf30a"
PARENT_BASE = "068815555a266b784bafc8dd521ca618eabbf30a"


def main() -> None:
    if os.environ.get("GITHUB_ACTIONS") != "true" or not os.environ.get(
        "GITHUB_REF", ""
    ).startswith("refs/heads/fix/r9-u12c-outbox-pause-"):
    ...
    network, database, runner = [f"ceiba-r9-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r9=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r9") != nonce:
    ...
    try:
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r9-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            'ENTRYPOINT ["python", "-m", "scripts.quality.r9.driver"]\n'
        )
        image = "ceiba-quality-r9:" + nonce
        ...
        for offset in range(0, len(encoded), 12000):
            print("R9_ZIP:" + encoded[offset:offset + 12000])
        print("R9_ZIP_SHA256:" + hashlib.sha256(archive.read_bytes()).hexdigest())
        print(
            json.dumps(
                {
                    k: manifest.get(k)
