...
PARENT_BASE = "4c767e029709354864c36767dfb25cbe1c95931d"
...
def main() -> None:
    ...
    network, database, runner = [f"ceiba-r11-{name}-{nonce}" for name in ("net", "db", "run")]
    resources = []
    label = "ceiba.quality.r11=" + nonce
    exit_code = 1
    ...
    def owned(kind: str, name: str) -> dict[str, Any]:
        ...
        if labels.get("ceiba.quality.r11") != nonce:
    ...
    try:
        ...
        root = Path(tempfile.mkdtemp(prefix="ceiba-r11-"))
        source = root / "source"
        source.mkdir()
        ...
        (root / "Dockerfile").write_text(
            ...
            "--report /dependency-install.json -r requirements-ci.lock\n"
            "ENV PYTHONDONTWRITEBYTECODE=1\n"
            'ENTRYPOINT ["python", "-m", "scripts.quality.r11.driver"]\n'
        )
        image = "ceiba-quality-r11:" + nonce
        ...
        for offset in range(0, len(encoded), 12000):
            print("R11_ZIP:" + encoded[offset : offset + 12000])
        print("R11_ZIP_SHA256:" + hashlib.sha256(archive.read_bytes()).hexdigest())
        print(
            json.dumps(
                {
                    k: manifest.get(k)
