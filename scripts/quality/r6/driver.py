"""R6 candidate tests with retained R1/R2/R3 nodes and independent Alembic regressions."""
...
def main() -> None:
    ...
    try:
        ...
        required = set(json.loads(Path("scripts/quality/r6/r5_nodes.json").read_text()))
        ...
        required_r6 = set(json.loads(Path("scripts/quality/r6/new_nodes.json").read_text()))
        discovered_r6 = {n for n in result["nodes"] if n.startswith("tests/remediation/r6/")}
        summary["new_r6_nodes"] = sorted(discovered_r6)
        summary["missing_red_nodes"] = sorted(required_r6 - discovered_r6)
        expected_nodes |= discovered_r6 | required_r6
