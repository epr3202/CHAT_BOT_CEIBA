...
from app.conversation.presentation import format_date_natural, format_event_type
...
def test_quote_summary_variables_present_event_type_and_date_in_natural_spanish() -> None:
    event_type = format_event_type("WEDDING")
    event_date = format_date_natural(date(2026, 9, 13))

    assert event_type == "una boda"
    assert event_date == "13 de septiembre de 2026"
    assert event_type not in EVENT_TYPES
    assert event_date != "2026-09-13"


@pytest.mark.parametrize("event_type", EVENT_TYPES)
def test_every_event_type_has_natural_spanish_presentation(event_type: str) -> None:
    label = format_event_type(event_type)
