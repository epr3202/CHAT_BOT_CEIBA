...
class Outbox(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("message_kind IN ('TEXT', 'DOCUMENT')", name="ck_outbox_message_kind"),
        CheckConstraint(
            "message_kind != 'DOCUMENT' OR catalog_asset_id IS NOT NULL",
            name="ck_outbox_document_has_catalog_asset",
        ),
    ...
    catalog_asset_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("catalog_asset.catalog_asset_id"), nullable=True
    )
    ...
    catalog_asset: Mapped[CatalogAsset | None] = relationship()

    @validates("channel")
    def validate_channel(self, key: str, channel: str | Channel) -> str:
        return Channel(channel).value

    @validates("message_kind")
    def validate_message_kind(self, key: str, value: str) -> str:
        if value not in {"TEXT", "DOCUMENT"}:
            raise ValueError(f"Invalid outbox message_kind: {value}")
        return value
